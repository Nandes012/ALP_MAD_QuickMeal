...
import { useFocusEffect, useRouter, useGlobalSearchParams } from 'expo-router'; // 👈 Diubah ke useGlobalSearchParams agar referensi objek stabil
...
export default function ListScreen() {
  ...
  const params = useGlobalSearchParams(); // 👈 Menangkap parameter navigasi global secara aman
  ...
  // 🛠️ FIX INFINITE LOOP: Validasi super ketat berbasis string, bukan objek param langsung
  useEffect(() => {
    const targetTab = params?.activeTab;
    if (targetTab === 'Bahan' || targetTab === 'Masak') {
      // Hanya ganti State jika tab aktif saat ini berbeda dengan tab tujuan kiriman detail resep
      if (activeTab !== targetTab) {
        setActiveTab(targetTab);
      }
    }
  }, [params?.activeTab]); // 👈 Cukup pantau string 'activeTab', jangan pantau seluruh objek 'params' maupun 'activeTab' lokal!
